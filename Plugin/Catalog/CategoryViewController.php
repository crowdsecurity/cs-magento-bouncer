...
/**
 * Plugin to handle log before viewing uncached category page
 */
class CategoryViewController extends Event implements EventInterface
{

    /**
     * @var string
     */
    protected $type = 'PRODUCT_LIST_VIEW';

    public function getEventData($objects = []): array
    {
        $controller = $objects['controller'] ?? null;
        return $controller ? ['product_list_id' => (string)$controller->getRequest()->getParam('id')] : [];
    }
